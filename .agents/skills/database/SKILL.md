...
- **Schema source:** `shared/schema/` directory (Drizzle ORM table definitions, re-exported from `shared/schema/index.ts`)
...
├── country-risk-premium-001.ts
├── db-hygiene-001.ts
├── documents-001.ts
├── drop-plaid-001.ts
├── export-config-001.ts
...
├── rebecca-chat-engine-001.ts
├── research-config-001.ts
├── role-partner-to-user-001.ts
├── seed-defaults-001.ts
└── themes-system-flag-001.ts
```
...
1. **Add the column/table to the appropriate file in `shared/schema/`** first (Drizzle schema is the source of truth)
...
- The schema in `shared/schema/` is always the source of truth for table structure

## Key Files

| File | Purpose |
|------|---------|
| `shared/schema/` | Drizzle ORM schema definitions (source of truth) |
