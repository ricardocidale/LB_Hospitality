...
Business simulation portal for **Hospitality Business Group**. Models a boutique hospitality management company alongside individual property SPVs with monthly and yearly financial projections. GAAP-compliant (ASC 230, ASC 360, ASC 470). 1,007 source files, ~165K lines, 3,915 tests across 168 test files. Hosted on Replit.
...
| Proof System | `.claude/skills/proof-system/SKILL.md` | 3,915 tests across 168 files, 583 golden values, verification commands |
...
## Testing & Proof System (3,915 Tests, 168 Files)
...
**Commands**: `npm test` (all 3,915 tests, 168 files) · `npm run verify` (8-phase GAAP) · `npm run health` (tsc+tests+verify+doc harmony)
...
npm run test:summary   # All 3,915 tests, 168 files (~35s)
