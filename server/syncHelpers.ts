...
export interface ForceSyncResults {
  properties: { name: string; updated: string[] }[];
  globalAssumptions: { updated: string[] };
}

export async function runForceSync(storage: IStorage): Promise<ForceSyncResults> {
  const results: ForceSyncResults = {
    properties: [],
    globalAssumptions: { updated: [] },
  };

  const existingAssumptions = await storage.getGlobalAssumptions();
  if (existingAssumptions) {
    const gaUpdates: Record<string, unknown> = {};
    const gaFields: Record<string, unknown> = {
      fundingSourceLabel: "Funding Vehicle",
    };
    for (const [key, seedValue] of Object.entries(gaFields)) {
      const existing = (existingAssumptions as any)[key];
      if (existing !== seedValue) {
        gaUpdates[key] = seedValue;
        results.globalAssumptions.updated.push(key);
      }
    }
    if (Object.keys(gaUpdates).length > 0) {
      await storage.upsertGlobalAssumptions({ ...existingAssumptions, ...gaUpdates } as any);
    }
  }

  const SEED_PROPERTIES = [
    { ...SEED_PROPERTY_DEFAULTS, name: "The Hudson Estate", operatingReserve: 250000, willRefinance: "Yes", refinanceDate: "2029-12-01", refinanceLtv: 0.75, refinanceInterestRate: 0.09, refinanceTermYears: 25, refinanceClosingCostRate: 0.03, exitCapRate: 0.08 },
    { ...SEED_PROPERTY_DEFAULTS, name: "Eden Summit Lodge", operatingReserve: 250000, willRefinance: "Yes", refinanceDate: "2030-07-01", refinanceLtv: 0.75, refinanceInterestRate: 0.09, refinanceTermYears: 25, refinanceClosingCostRate: 0.03 },
    { ...SEED_PROPERTY_DEFAULTS, name: "Austin Hillside", operatingReserve: 250000, willRefinance: "Yes", refinanceDate: "2031-01-01", refinanceLtv: 0.75, refinanceInterestRate: 0.09, refinanceTermYears: 25, refinanceClosingCostRate: 0.03 },
    { ...SEED_PROPERTY_DEFAULTS, name: "Casa Medellín", operatingReserve: 600000, acquisitionLTV: 0.60, acquisitionInterestRate: 0.095, acquisitionTermYears: 25, acquisitionClosingCostRate: 0.02, exitCapRate: 0.095 },
    { ...SEED_PROPERTY_DEFAULTS, name: "Blue Ridge Manor", operatingReserve: 500000, acquisitionLTV: 0.60, acquisitionInterestRate: 0.09, acquisitionTermYears: 25, acquisitionClosingCostRate: 0.02, exitCapRate: 0.09 },
  ];
  ...
  const forceFields = [
    "operatingReserve", "refinanceLtv", "refinanceInterestRate",
    "refinanceTermYears", "refinanceClosingCostRate", "exitCapRate",
    "acquisitionLTV", "acquisitionInterestRate", "acquisitionTermYears", "acquisitionClosingCostRate",
  ];

  for (const seedProp of SEED_PROPERTIES) {
    const existing = existingByName.get(seedProp.name);
    ...
    for (const field of forceFields) {
      const seedValue = (seedProp as any)[field];
      if (seedValue === undefined) continue;
      const existingValue = (existing as any)[field];
      if (existingValue !== seedValue) {
        updates[field] = seedValue;
        updatedFields.push(`${field}: ${existingValue} → ${seedValue}`);
      }
    }
    ...
    results.properties.push({ name: seedProp.name, updated: updatedFields });
  }
