...
export async function runNotificationLogs001(): Promise<void> {
  const alterStatements = [
    ...
    `DO $$ BEGIN ALTER TABLE plaid_connections ADD CONSTRAINT plaid_connections_user_id_fk FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE; EXCEPTION WHEN duplicate_object THEN NULL; END $$`,
    `DO $$ BEGIN ALTER TABLE plaid_connections ADD CONSTRAINT plaid_connections_property_id_fk FOREIGN KEY (property_id) REFERENCES properties(id) ON DELETE CASCADE; EXCEPTION WHEN duplicate_object THEN NULL; END $$`,
    `DO $$ BEGIN ALTER TABLE plaid_transactions ADD CONSTRAINT plaid_transactions_connection_id_fk FOREIGN KEY (connection_id) REFERENCES plaid_connections(id) ON DELETE CASCADE; EXCEPTION WHEN duplicate_object THEN NULL; END $$`,
    `DO $$ BEGIN ALTER TABLE plaid_transactions ADD CONSTRAINT plaid_transactions_property_id_fk FOREIGN KEY (property_id) REFERENCES properties(id) ON DELETE CASCADE; EXCEPTION WHEN duplicate_object THEN NULL; END $$`,
