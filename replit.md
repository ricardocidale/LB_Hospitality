...
- **LLM dual-model config** — primary + fallback model with vendor selection (OpenAI, Anthropic, Gemini) in Admin LLM tab. 7 domains: Company Research, Property Research, Market Research, Report Generation, Chatbot (Rebecca), Premium Exports, AI Utilities. Tab-level defaults (vendor + model) cascade to all cards in that tab; domain cards can override. Resolution order: domain explicit → tab default → hardcoded default. All AI model selections defined in admin LLMs page only — no hardcoded models anywhere. **Zod sync rule:** when adding fields to `ResearchConfig`, update all 3 places: TypeScript interface, server Zod schema (`.strict()`), and merge handler. See `.agents/skills/zod-schema-sync/SKILL.md`.
