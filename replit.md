...
-   **Financial Accuracy & Compliance:** Highest priority, enforced by a comprehensive proof system (3,665 tests across 161 files), GAAP verification, and USALI 12th Edition compliance for property-level Income and Cash Flow Statements. The Balance Sheet Identity (A = L + E) must hold within $1.
...
-   **Deep Audit Tests:** 4 audit test files in `tests/audit/`: data-flow integrity (engine pipeline trace, precision, determinism), cache invalidation (hit/miss/clear correctness), scenario save/load (roundtrip hash, consolidation, property-order invariance), endpoint security (auth coverage, rate limiting, Zod validation grep).
...
| Proof System | `.claude/skills/proof-system/SKILL.md` | 3,665 tests, verification commands |
...
npm run test:summary   # All 3,665 tests, 161 files (~35s)
