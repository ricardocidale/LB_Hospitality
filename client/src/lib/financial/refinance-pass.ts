...
export function applyRefinancePostProcessing(
  ...
  const refiMonthIndex = (refiDate.getFullYear() - ctx.modelStart.getFullYear()) * 12 +
  ...
  const acqMonthIdx = (ctx.acquisitionDate.getFullYear() - ctx.modelStart.getFullYear()) * 12 +
