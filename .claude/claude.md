...
Business simulation portal for **Hospitality Business Group**. Models a boutique hospitality management company alongside individual property SPVs with monthly and yearly financial projections. GAAP-compliant (ASC 230, ASC 360, ASC 470). ~673 source files, ~103,018 lines, 2,820 tests across 122 files. Hosted on Replit.
...
- **TOP PRIORITY: Financial accuracy always beats UI enhancements.** The 2,820-test proof system must always pass.
...
| Proof System | `.claude/skills/proof-system/SKILL.md` | 2,820 tests, 418 golden tests, verification commands |
...
## Testing & Proof System (2,820 Tests, 122 Files)
...
**Commands**: `npm test` (all 2,820) · `npm run verify` (7-phase GAAP) · `npm run health` (tsc+tests+verify)
...
npm run test:summary   # All 2,820 tests, 1-line output
