...
Global assumptions: Inflation ${(globalAssumptions.inflationRate * 100).toFixed(1)}%, Base fee ${(globalAssumptions.baseManagementFee * 100).toFixed(1)}%, Incentive fee ${(globalAssumptions.incentiveManagementFee * 100).toFixed(1)}%`;
  ...
  app.post("/api/returns/dcf-npv", requireAuth, async (req, res) => {
    try {
      const parsed = dcfSchema.safeParse(req.body);
      if (!parsed.success) return res.status(400).json({ error: fromZodError(parsed.error).message });
      const { computeDCF } = await import("../calc/returns/dcf-npv");
      const result = computeDCF({ ...parsed.data, rounding_policy: defaultRounding });
      res.json(result);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  const irrVectorSchema = z.object({
  ...
  app.post("/api/returns/irr-vector", requireAuth, async (req, res) => {
    try {
      const parsed = irrVectorSchema.safeParse(req.body);
      if (!parsed.success) return res.status(400).json({ error: fromZodError(parsed.error).message });
      const { buildIRRVector } = await import("../calc/returns/irr-vector");
      const result = buildIRRVector(parsed.data);
      res.json(result);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  const equityMultipleSchema = z.object({
    cash_flows: z.array(z.number()).min(1),
    label: z.string().optional(),
  });

  app.post("/api/returns/equity-multiple", requireAuth, async (req, res) => {
    try {
      const parsed = equityMultipleSchema.safeParse(req.body);
      if (!parsed.success) return res.status(400).json({ error: fromZodError(parsed.error).message });
      const { computeEquityMultiple } = await import("../calc/returns/equity-multiple");
      const result = computeEquityMultiple({ ...parsed.data, rounding_policy: defaultRounding });
      res.json(result);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  const exitValuationSchema = z.object({
  ...
  app.post("/api/returns/exit-valuation", requireAuth, async (req, res) => {
    try {
      const parsed = exitValuationSchema.safeParse(req.body);
      if (!parsed.success) return res.status(400).json({ error: fromZodError(parsed.error).message });
      const { computeExitValuation } = await import("../calc/returns/exit-valuation");
      const result = computeExitValuation({ ...parsed.data, rounding_policy: defaultRounding });
      res.json(result);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  const financialIdentitiesSchema = z.object({
  ...
  app.post("/api/validation/financial-identities", requireChecker, async (req, res) => {
    try {
      const parsed = financialIdentitiesSchema.safeParse(req.body);
      if (!parsed.success) return res.status(400).json({ error: fromZodError(parsed.error).message });
      const { validateFinancialIdentities } = await import("../calc/validation/financial-identities");
      const result = validateFinancialIdentities({ ...parsed.data, rounding_policy: defaultRounding });
      res.json(result);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  const fundingGatesSchema = z.object({
  ...
  app.post("/api/validation/funding-gates", requireChecker, async (req, res) => {
    try {
      const parsed = fundingGatesSchema.safeParse(req.body);
      if (!parsed.success) return res.status(400).json({ error: fromZodError(parsed.error).message });
      const { checkFundingGates } = await import("../calc/validation/funding-gates");
      const result = checkFundingGates(parsed.data);
      res.json(result);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  const scheduleReconcileSchema = z.object({
    property_name: z.string().optional(),
  ...
  app.post("/api/validation/schedule-reconcile", requireChecker, async (req, res) => {
    try {
      const parsed = scheduleReconcileSchema.safeParse(req.body);
      if (!parsed.success) return res.status(400).json({ error: fromZodError(parsed.error).message });
      const { reconcileSchedule } = await import("../calc/validation/schedule-reconcile");
      const result = reconcileSchedule(parsed.data);
      res.json(result);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  const assumptionConsistencySchema = z.object({
  ...
  app.post("/api/validation/assumption-consistency", requireChecker, async (req, res) => {
    try {
      const parsed = assumptionConsistencySchema.safeParse(req.body);
      if (!parsed.success) return res.status(400).json({ error: fromZodError(parsed.error).message });
      const { checkAssumptionConsistency } = await import("../calc/validation/assumption-consistency");
      const result = checkAssumptionConsistency(parsed.data);
      res.json(result);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.post("/api/validation/export-verification", requireChecker, async (req, res) => {
    try {
      const parsed = exportVerificationSchema.safeParse(req.body);
      if (!parsed.success) return res.status(400).json({ error: fromZodError(parsed.error).message });
      const { verifyExport } = await import("../calc/validation/export-verification");
      const result = verifyExport(parsed.data);
      res.json(result);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.post("/api/analysis/consolidation", requireChecker, async (req, res) => {
    try {
      const parsed = consolidationSchema.safeParse(req.body);
      if (!parsed.success) return res.status(400).json({ error: fromZodError(parsed.error).message });
      const { consolidateStatements } = await import("../calc/analysis/consolidation");
      const result = consolidateStatements({ ...parsed.data, rounding_policy: defaultRounding });
      res.json(result);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.post("/api/analysis/scenario-compare", requireChecker, async (req, res) => {
    try {
      const parsed = scenarioCompareSchema.safeParse(req.body);
      if (!parsed.success) return res.status(400).json({ error: fromZodError(parsed.error).message });
      const { compareScenarios } = await import("../calc/analysis/scenario-compare");
      const result = compareScenarios(parsed.data);
      res.json(result);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.post("/api/analysis/break-even", requireChecker, async (req, res) => {
    try {
      const parsed = breakEvenSchema.safeParse(req.body);
      if (!parsed.success) return res.status(400).json({ error: fromZodError(parsed.error).message });
      const { computeBreakEven } = await import("../calc/analysis/break-even");
      const result = computeBreakEven(parsed.data);
      res.json(result);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
