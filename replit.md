...
- **USALI Income Statement Restructure** — Restructured the Consolidated Income Statement to follow USALI 12th Edition order. Split Operating Expenses into Departmental and Undistributed sections. Added AGOP (Adjusted Gross Operating Profit) chevron row. Added Debt Service section (Interest, Principal, DSCR). Added Net Income section (Depreciation, Tax, Cash Flow). Added TRevPAR, GOPPAR, GOP Margin, NOI Margin to metrics. NOI now uses engine value directly (`noi = agop − expenseTaxes`), deprecating old `display_NOI = noi + feeBase + feeIncentive`. Waterfall chart updated to match USALI flow. Skill: `.agents/skills/usali-income-statement/SKILL.md`.
