...
>) => {
  ...
  return (
    ...
    >
      <SliderPrimitive.Track className="bg-muted relative h-[3px] w-full grow overflow-hidden rounded-full">
      ...
      >
        <div className="bg-foreground absolute size-2.5 rounded-full" />
      </SliderPrimitive.Thumb>
    </SliderPrimitive.Root>
  )
}
...
type AnimFrameCallback = (delta: number) => void

function useAnimationFrame(callback: AnimFrameCallback) {
  const requestRef = useRef<number | null>(null)
  const previousTimeRef = useRef<number | null>(null)
  const callbackRef = useRef<AnimFrameCallback>(callback)
