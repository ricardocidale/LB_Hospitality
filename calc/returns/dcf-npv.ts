...
export function computeDCF(input: DCFInput): DCFOutput {
  ...
  let npv = 0;
  let total_undiscounted = 0;

  for (let t = 0; t < input.cash_flows.length; t++) {
    const cf = input.cash_flows[t];
    total_undiscounted += cf;
    const pv = cf / Math.pow(1 + input.discount_rate, t);
  ...
  return {
    npv: r(npv),
    pv_timeline,
    total_undiscounted: r(total_undiscounted),
    annualized_discount_rate: roundTo(annualized_discount_rate, { precision: 6, bankers_rounding: false }),
