...
**Codebase:** ~530 source files, ~79,000 lines of code, 1,946 tests across 99 files.
...
- **TOP PRIORITY: Calculations and correct reports are always the highest priority.** Financial accuracy must never be compromised for visual or UI enhancements. The automated proof system (1,946 tests) must always pass.
...
| Proof System | `.claude/skills/proof-system/SKILL.md` | 1,946 tests, 5 golden scenarios, verification commands |
...
npm run test:summary   # Run all 1,946 tests, 1-line output on pass
npm run verify:summary # 6-phase verification, compact output
npm test               # Run all 1,946 tests (full output)
