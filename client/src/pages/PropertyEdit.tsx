...
export default function PropertyEdit() {
  ...
  const researchValues = (() => {
    const GENERIC_DEFAULTS: Record<string, { display: string; mid: number; source?: string }> = {
      ...
      svcFeeTechReservations: { display: "1.5%–3.0%", mid: 2 },
      svcFeeAccounting: { display: "0.5%–1.5%", mid: 1 },
      svcFeeRevenueMgmt: { display: "0.5%–1.5%", mid: 1 },
    ...
    const aiValues: Record<string, { display: string; mid: number } | null> = {
      ...
      svcFeeTechReservations: parseCostRate(msf?.serviceFeeCategories?.technology_reservations || msf?.serviceFeeCategories?.technologyReservations || msf?.serviceFeeCategories?.it || msf?.serviceFeeCategories?.reservations),
      svcFeeAccounting: parseCostRate(msf?.serviceFeeCategories?.accounting),
      svcFeeRevenueMgmt: parseCostRate(msf?.serviceFeeCategories?.revenue_management || msf?.serviceFeeCategories?.revenueManagement),
