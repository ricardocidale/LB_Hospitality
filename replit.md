...
Business simulation portal for **Hospitality Business Group**. Models a boutique hospitality management company alongside individual property SPVs with monthly and yearly financial projections. GAAP-compliant (ASC 230, ASC 360, ASC 470). 889 source files, ~157K lines, 3,439 tests across 154 test files. Hosted on Replit.
...
| Proof System | `.claude/skills/proof-system/SKILL.md` | 3,439 tests, 583 golden tests, verification commands |
...
## Testing & Proof System (3,439 Tests, 154 Files)
...
**Commands**: `npm test` (all 3,439) · `npm run verify` (7-phase GAAP) · `npm run health` (tsc+tests+verify)
...
- **LLM dual-model config** — primary + fallback model with vendor selection (OpenAI, Anthropic, Gemini) in Admin LLM tab. 7 domains: Company Research, Property Research, Market Research, Report Generation, Chatbot (Rebecca), Premium Exports, AI Utilities. LLM defaults live in **App Defaults → LLM Defaults** tab (not inline on the LLMs page). Resolution order: card-level explicit → tab default (App Defaults) → hardcoded default. **Zod sync rule:** when adding fields to `ResearchConfig`, update all 3 places: TypeScript interface, server Zod schema (`.strict()`), and merge handler. See `.agents/skills/zod-schema-sync/SKILL.md`.
- **App Defaults** — ALL seed/starting values live exclusively in Admin → System → App Defaults. Three sub-tabs: Market & Macro, Property Underwriting, LLM Defaults. No other page may define inline default selectors. See `.agents/skills/app-defaults/SKILL.md`.
...
## Balance Sheet Identity Rule

**A = L + E must hold within $1 on every balance sheet.** Cash derivation must always use `m.anoi` (not `m.noi`) because `cashFlow = ANOI − debtPayment − incomeTax`. Using NOI overstates cash by cumulative FF&E reserve. See `rules/balance-sheet-identity.md` and `skills/balance-sheet-integrity/SKILL.md`.
...
npm run test:summary   # All 3,439 tests, 1-line output (~35s)
...
- **USALI Income Statement Restructure (All Views)** — Restructured ALL property-level Income Statements to follow USALI 12th Edition order: Revenue → Dept Expenses → Undistributed Expenses → GOP → Mgmt Fees → AGOP → Fixed Charges → NOI → FF&E → ANOI → Debt Service → Net Income. Applied to: Consolidated IS (`dashboard/IncomeStatementTab`), Property IS (`statements/YearlyIncomeStatement`), Property Detail IS tab (waterfall + line chart), Monthly IS (`FinancialStatement`), Excel exports, PDF exports, Checker Manual exports. NOI now uses `engine.noi` directly everywhere (deprecated `noi + feeBase + feeIncentive`). AGOP added to all line charts with `#10B981` color. Company IS (`CompanyIncomeTab`) intentionally NOT restructured — it's a management company P&L, not a USALI property IS. Skill: `.agents/skills/usali-income-statement/SKILL.md`.
- **USALI Cash Flow Statement Restructure (All Views)** — Applied USALI-aligned expense grouping to all property-level Cash Flow Statements: CFO operating expenses now split into Departmental Expenses → Undistributed Operating Expenses → Management Fees (matching IS). Applied to: Property CF (`statements/YearlyCashFlowStatement`), Excel CF exports (`property-sheets.ts`), Checker Manual CF export. Fixed FCF formula display bug in Dashboard CF (`CashFlowTab.tsx`) — was showing "ANOI" but using NOI variable; now correctly shows "CFO − CapEx". Added AGOP row to Monthly IS (`FinancialStatement.tsx`). Added ANOI and DSCR to checker CF export. Company CF (`CompanyCashFlowTab`) intentionally NOT restructured — management company CF. SKILL.md updated with full CF section order and formulas.
